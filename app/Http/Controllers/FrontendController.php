...
class FrontendController extends Controller
{
    ...
    public function list_hotels($keyword)
    {
        ...
        return view('frontend.list_hotels', compact('hotels', 'keyword'));
    }

    public function hotel_details(Hotel $hotel)
    {
        $latestPhotos = $hotel->photos()->orderByDesc('id')->take(3)->get();
        return view('frontend.hotels_detail', compact('hotel', 'latestPhotos'));
    }

    public function hotel_details_rooms(Hotel $hotel)
    {
        return view('frontend.list_hotels_room', compact('hotel'));
    }
    ...
    public function hotel_room_book(StoreHotelBookingRequest $request, Hotel $hotel, HotelRoom $hotel_room)
    {
        ...
        // "&" pada closure ditambahkan supaya $hotelBookingId = null nanti bisa berubah
        ...
        return redirect()->route('frontend.hotel.book.payment', $hotelBookingId);
    }

    public function hotel_payment(HotelBooking $hotel_booking)
    {
        $user = Auth::user();
        return view('frontend.book_payment', compact('hotel_booking', 'user'));
    }
